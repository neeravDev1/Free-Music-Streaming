...
const TopChartCard = ({song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick})=>{
  return (<div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg
  cursor-pointer mb-2">
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      ...
      <div className="flex-1 flex flex-col justify-center mx-3">
        ...
        {/* {song?.artists?.length>0 && <Link to={`artists/${song?.artists[0].adamid}`}> */}
          <p className="text-ase text-gray-300 mt-1" >{song?.subtitle}</p>
        {/* </Link>} */}
      </div>
    </div>
    {console.log("temp", handlePlayClick)}
...
const handlePlayClick = (song, i)=>{
 ...
 const topPlays = data?.result.tracks?.slice(5);
   ...
   flex-1 xl:max-w-[500px] max-w-full flex flex-col" >
    <div className="w-full flex flex-col">
      ...
      <div className="mt-4 flex flex-col gap-1">
        {topPlays?.map((song, i)=>{
          console.log("handle", handlePlayClick)
          return <TopChartCard key={song.key} song={song} i={i} isPlaying={isPlaying} activeSong={activeSong}
    ...
    <div className="w-full flex flex-col mt-8">
      ...
      style={{ overflow: 'scroll' }}>
        {topPlays?.map((song, i)=> { 
          ...
          return <SwiperSlide key={song?.key} style={
